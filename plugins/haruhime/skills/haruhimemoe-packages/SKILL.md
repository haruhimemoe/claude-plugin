...
description: Use when building an osu! tournament tool on the @haruhimemoe npm packages (pools.haruhime.moe, sheets.haruhime.moe, packs.haruhime.moe or your own), choosing which package does a job, deciding what may run in the browser, edge or build time, or installing them
---
...
- All are ESM only, ship their own types and need Node 22.12 or later. `pool`, `compliance`, `hinai` and `@haruhimemoe/osu/shapes` also run in Bun, browsers and edge runtimes. The `@haruhimemoe/osu` root is server-only (it holds your secret), and `brand` is build-time only (native PNG rendering).
