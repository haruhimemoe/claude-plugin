...
- `public/brand/pools-wordmark.svg` (dark backgrounds), `pools-wordmark-on-light.svg` (light backgrounds), `pools-icon.svg` (schema.org `logo`), `pools-palette.json`.
- `src/app/icon.svg` (favicon), `src/app/apple-icon.png` (180×180, square: iOS rounds it), `src/app/opengraph-image.png` and its `.alt.txt`.

Next.js serves the `app/` files by name, so there's no route code and nothing renders per request. Commit the output. Rerun after upgrading the package; it prints `wrote` or `replaced` per file. `--dry-run` lists the paths first. Other options: `--root`, `--public`, `--app` (both must be inside `--root`, and the app directory must already exist).
...
Importing the package doesn't load the native PNG renderer; only `svgToPng` (and the CLI) does, so palettes and SVGs work even where it isn't installed. Still, keep it to build time and Node 22.12+, never a browser or edge runtime. `palette` and `brandFiles` throw `RangeError` for a hue outside 0–359 or a name that isn't lowercase `a-z0-9-`. The bundled fonts are subset to printable ASCII. Any other character (accents, CJK, tabs, non-breaking spaces) throws instead of drawing a blank box, so product names and marks stay ASCII.
